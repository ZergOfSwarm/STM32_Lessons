...
char *name_sd = "config.txt";       // Имя будущего файла
//char *folder_name = "FOLDER-15";  // Имя будущей папки
...
char rtext[400];                  // Массив в который будем сохранять данные из файла при чтении.12500
...
void zerg_get_file_info(char *name) {
	...
	case FR_OK:
		...
		// send_uart(buffer);
		printf("Timestamp: %u/%02u/%02u, %02u:%02u\n", (fileinfo.fdate >> 9) + 1980, fileinfo.fdate >> 5 & 15, fileinfo.fdate & 31, fileinfo.ftime >> 11, fileinfo.ftime >> 5 & 63);
		// end_uart(buffer);
		printf("Attributes: %c%c%c%c%c\n", (fileinfo.fattrib & AM_DIR) ? 'D' : '-', (fileinfo.fattrib & AM_RDO) ? 'R' : '-', (fileinfo.fattrib & AM_HID) ? 'H' : '-', (fileinfo.fattrib & AM_SYS) ? 'S' : '-', (fileinfo.fattrib & AM_ARC) ? 'A' : '-');
		// send_uart(buffer);
		break;
